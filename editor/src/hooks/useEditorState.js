...
const useEditorState = (id, userName) => {
  ...
  const updateStateFromDoc = (doc) => {
    ...
    const { tree, contents, input, output, lang: docLang, roomMode, projectLanguage, config } = doc.data;
  ...
  const actions = {
    ...
    updateConfig: async (newConfig) => {
      if (!stateRef.current.docLoaded || !docRef.current) return false;
      ...
      try {
        ...
        setState(s => ({ ...s, config: merged, roomMode: merged.roomMode || s.roomMode, projectLanguage: merged.projectLanguage || s.projectLanguage }));
        return true;
      } catch (e) {
  ...
  return { state, actions: actionsRef, handleSaveToWorkspace, openTerminalInWorkspace };
};
