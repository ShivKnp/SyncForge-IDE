...
const TerminalPanel = ({ sessionId, visible = false, onToggle, onClose }) => {
  ...
  const defaultHeight = 280; // Reduced from 340px to 280px
  const minHeight = 120;
  ...
  useEffect(() => {
    ...
    // If height is very small (due to zoom/old value), compute a better default based on viewport
    const winH = window.innerHeight;
    const suggested = Math.max(defaultHeight, Math.floor(winH * 0.28)); // Reduced from 32% to 28%
    setHeightPx(prev => {
    ...
    // fit + focus after layout settles
    setTimeout(() => {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      setStatus('connected');
      setConnected(true);
      try { term.writeln('\x1b[32mConnected to terminal.\x1b[0m'); } catch(e){}
    ...
    ws.onclose = () => { setStatus('closed'); setConnected(false); try { term.writeln('\x1b[33mTerminal disconnected.\x1b[0m'); } catch(e){}; };
    ws.onerror = () => { setStatus('error'); setConnected(false); };
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: collapsed ? 'height 160ms ease' : 'none', // Only animate when collapsing
        display: 'flex',
